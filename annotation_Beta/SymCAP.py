...
parser = argparse.ArgumentParser(description="Symbiosis Custom Annotation Pipeline")
parser.add_argument("--work_dir","--wd", metavar='<str>',type=str, help="working directory path")
parser.add_argument("--genomes_for_annotation_dir","--genomes_dir", metavar='<str>',type=str, help="path to folder with genomes to annotate")
parser.add_argument("--protein_ref_dir", "--prot_db", metavar='<str>', type=str, help="path to folder with protein-coding gene alignments")
parser.add_argument("--rRNA_ref_dir", "--RNA_ref", metavar='<str>', type=str, help="path to folder with rRNA, tmRNA and ncRNA gene alignments")
parser.add_argument("--gene_detail_table", "--gene_list", metavar='<str>', type=str, help="path to gene detail table")
...
parser.add_argument('-i', '--input_dir', required=True, help='Input directory containing genomes to annotate')
parser.add_argument('-o', '--output_dir', required=True, help='Output directory for annotation results')
parser.add_argument('-t', '--threshold', type=float, default=0.6, help='Threshold for functional genes (default: 0.6)')
parser.add_argument('-p', '--putative', type=float, default=0.9, help='Threshold for putative genes (default: 0.9)')
parser.add_argument('-u', '--upper', type=float, default=1.5, help='Upper threshold for functional genes (default: 1.5)')
parser.add_argument('-s', '--size', type=str, default='RANSCY', help='Size reference for gene length normalization (default: RANSCY)')
parser.add_argument('-tt', '--table', type=int, default=4, help='Translation table for gene prediction (default: 4)')

args = parser.parse_args()

input_dir = args.input_dir
output_dir = args.output_dir
Threshold_functional = args.threshold
Threshold_putative = args.putative
Upper_threshold_functional = args.upper
Size_reference = args.size
...
genomes_for_annotation_dir = args.genomes_for_annotation_dir
protein_ref_dir = args.protein_ref_dir
rRNA_ref_dir = args.rRNA_ref_dir
gene_detail_table = args.gene_detail_table
...
if "B2" in Blocks_to_run:
    ...
    for genome in genome_list:
        ...
        if not glob.glob("%s/%s_orfs.fasta" % (genome_path, genome)):
            out_gff = "%s.temp.gff" % genome_path
            orfs_fasta = "%s/%s_orfs.fasta" % (genome_path, genome)
            fasta_input = "%s/%s.fasta" % (genomes_for_annotation_dir, genome)
            cmd = "pyrodigal -g %s -a %s -i %s -o %s -f gff" % (table, orfs_fasta, fasta_input, out_gff)
            subprocess.check_call(cmd.split())
            
            # rename the sequences in the orfs file
            with open(orfs_fasta) as infile, open("%s.temp.fasta" % genome_path, 'w') as outfile:
                for line in infile:
                    match = re.match(r'^>([^#]+)#\s+([0-9]+)\s+#\s+([0-9]+)\s+#\s+(-?\d+).+$', line)
                    if match:
                        ...
                        if strand == '-1':
                            ...
                            new_line = ">%s [%s - %s] %s\n" % (seq_name, end, start, strand)
                        else:
                            new_line = ">%s [%s - %s]\n" % (seq_name, start, end)
                        outfile.write(new_line)
                    else:
                        outfile.write(line)
            os.rename("%s.temp.fasta" % genome_path, orfs_fasta)
            print("    from genome " + genome + ", using ...........DONE!")
        else:
...
if "B12" in Blocks_to_run:
    print("\n\n######################## Executing Block 12 ####################################")
    print("Scanning genomes for tRNA genes using tRNAscan-SE v.2.0.9, adds them to newly created tRNA_gene_list\n")
...
#CP008699.1	Genbank	tRNA	46580	46652	.	+	.	ID=rna4;Parent=gene58;Note=COORDINATES: profile:Aragorn:1.2.34%2CtRNA-Trp(tca);gbkey=tRNA;product=tRNA-Trp
            ...
            if feature[1] == "tRNA":
                ...
                attribute2 += "note=inference:tRNAscan-SE v.2.0.9"
